...
class Day extends React.Component {
  ...
  componentWillMount() {
       ...
       tasksRef.on('value', snap => {
            ...
            this.setState({ tasks:   tasks, ChampsLoading: false });
        });
    }
...
let id = tasksRef.push().key;
  ...
  render() {
        ...
        const { tasks, ChampsLoading } = this.state;
    const reminders =
      _sortBy(this.props.reminders[this.props.date], "time") || [];

      if (ChampsLoading) {
            reminders = <div className="TaskList-empty">Loading...</div>;
